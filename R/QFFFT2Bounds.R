QFFFT2Bounds <- function(obs, M, mu, sigma, k = c(20), resid.op.norm.bd = NULL, lower.tail = TRUE, log = FALSE) {
  ...
  evecs <- e$vectors[, order(abs(e$values), decreasing=TRUE)]
  evals <- e$values[order(abs(e$values), decreasing=TRUE)]

  # Set resid.op.norm.bd to magnitude of smallest truncated eigenvalue if not specified
  if(is.null(resid.op.norm.bd)) {
    resid.op.norm.bd <- abs(evals[length(evals)])
  }
  ...
  for(kk in 1:length(k)) {
    ncps[[kk]] <- c(crossprod(evecs[,1:k[kk]], mu.tilde))^2
  }
  ...
  E_R <- list()
  for(kk in 1:length(k)) {
    R <- (M.tilde - evecs[,1:k[kk]] %*% (t(evecs[,1:k[kk]]) * evals[1:k[kk]]))
  ...
  for(kk in 1:length(k)) {
    for(i in 1:length(obs)) {
      res <- rbind(res, c(k = k[kk],
                          obs = obs[i],
                          QFFFT2Bounds2(obs[i], evals[1:k[kk]], ncps[[kk]], E_R[[kk]], nu[[kk]], N, resid.op.norm.bd, "missing", lower.tail, log)))
    }
  }
  res
}
