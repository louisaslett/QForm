#' Bounds the CDF of Quadratic Forms
#'
...
#' @param max.eta vector; element-wise upper bound on the absolute value of the \eqn{\eta_i} in \eqn{R_f} (see Details)
...
#'
#' @return A function which evaluates upper and lower bounds on the CDF of \eqn{Q_f}.
#'
#' @examples
#' x <- seq(-100,100)
...
QBounds <- function(cdf, f = "identity", max.eta, sum.eta, sum.etasq, sum.eta.deltasq = 0, sum.etasq.deltasq = 0) {
  ...
  if(f == "identity"){
    conc.ineqs <- WrapConcIneq.identity(sum.eta.deltasq + sum.eta,sum.eta.deltasq + sum.eta, 8*sum.etasq.deltasq + 4*sum.etasq, max.eta)
  }else{
