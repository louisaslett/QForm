QFFFTBounds <- function(obs, M, mu, sigma, k = c(20), resid.op.norm.bd = NULL, lower.tail = TRUE, log = FALSE) {
  ...
  for(kk in 1:length(k)) {
    for(i in 1:length(obs)) {
      res <- rbind(res, c(k = k[kk],
                          obs = obs[i],
                          QFFFTBounds2(obs[i], eval.tilde[1:k[kk]], ncps[[kk]], E_R[[kk]], nu[[kk]], N, resid.op.norm.bd, "missing", lower.tail, log)))
    }
  }
  res
}
