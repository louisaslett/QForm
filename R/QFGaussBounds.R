...
#' @export
QFGaussBoundsNew2 <- function(cdf, f = "identity", max.abs.eta, sum.eta, sum.etasq, sum.eta.deltasq = 0, sum.etasq.deltasq = 0) {
  ...
  # Create bounding function
  function(q = NULL, ql = NULL, qu = NULL){
    ...
    # upper.components.sorted <- sort(upper.components)
    # lower.components.sorted <- sort(lower.components)
    #
    #
    # ans <- QFRestrictInterval(c(sum(lower.components.sorted),
    #                           sum(upper.components.sorted),
    #                           sum(c(1,-rev(lower.components.sorted))),
    #                           sum(c(1,-rev(upper.components.sorted)))))
    #
