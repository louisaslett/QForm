Tcdf <- function(evals, ncps, n = 2^16-1, num.baseline.intervals = 20, min.interval.length = 20, qfft.apply = sapply) {
  ...
  rho <- qfft.apply(1:n, function(k, a.standardized, b.standardized, n, evals.tilde, ncps) {
    exp(log.rho.func((pi*n/(b.standardized-a.standardized))*(2*((k-1)/n)-1), evals.tilde, ncps, a.standardized)) / (pi*(2*((k-1)/n)-1))
  ...
  r0 <- which( fft_cdf[tstart.r:n]<=0 )
  best.r <- ifelse(length(r0)==0, n, tstart.r-2+min(r0))
  l0 <- which( fft_cdf[1:tstart.l]<=0 )
  ...
  rblips <- which(diff(log.cdf.r[tstart.r:best.r]) < -sqrt(.Machine$double.eps))
  best.r <- ifelse(length(rblips)==0,best.r,tstart.r-2+min(rblips))
  lblips <- which(diff(log.cdf.l[best.l:tstart.l]) > sqrt(.Machine$double.eps))
  ...
  r2.r <- calc.R2(log.cdf.r[ctstart.r:best.r], xx.standardized[ctstart.r:best.r], interval.length.r)
  ...
  r2.l <- calc.R2(log.cdf.l[ctstart.l:best.l], xx.standardized[ctstart.l:best.l], interval.length.l)
  ...
  a.r <- log.cdf.r[best.r]-xx[best.r]*b.r#+log(SD.max)
  a.l <- log.cdf.l[best.l]-xx[best.l]*b.l#+log(SD.max)
